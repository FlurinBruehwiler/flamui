...
namespace TolggeUI.SourceGenerators;
...
public class RenderObjectGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        var parameterAttributeType = context.Compilation.GetTypeByMetadataName("TolggeUI.ParameterAttribute");
        var extensionAttributeType = context.Compilation.GetTypeByMetadataName("TolggeUI.ExtensionAttribute");
        var componentInterface = context.Compilation.GetTypeByMetadataName("TolggeUI.Component");
        _actionType = context.Compilation.GetTypeByMetadataName(typeof(Action).FullName);

        if (parameterAttributeType is null)
            throw new Exception("TolggeUI.ParameterAttribute not found");

        if (extensionAttributeType is null)
            throw new Exception("TolggeUI.InheritAttribute not found");

        if (componentInterface is null)
            throw new Exception("TolggeUI.Component not found");
